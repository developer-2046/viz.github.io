# Inference Sheet: Expanded Disability Status Scale (Rank 3)
**Grouping**: Low(<25%) vs High(>75%)
**Subset**: Residence=Edinburgh
**Metrics**: AUC=0.67, KNN=0.64, CentroidDist=0.08

## A) Plain English Summary
This plot shows the relationship between microbiome composition and Group A/B defined by Expanded Disability Status Scale. There is substantial overlap between the groups, suggesting that group membership is associated with moderate differences in composition. Dispersions are similar.

## B) Supported Inferences
- There is an association between the axes and the grouping variable (AUC = 0.67).
...
> **Correct Answer**: B
> *Rationale*: The KNN purity of 0.64 indicates overlap, but AUC 0.67 suggests signal.
